...
function createWindow() {
  ...
  const sendUploadProgress = (p) =>
    mainWindow.webContents.send("uploadProgress", p);

  ipcMain.on("uploadFiles", async (event, filePaths) => {
    /** @type {string} */
    ...
    fs.readdir(filePaths[0], (err, files) => {
      files.forEach(async (fileName) => {
        const paths = [path.join(filePaths[0], fileName)];
        try {
          ...
          try {
            ...
            if (
              paths.length === 1 &&
              (await fs.promises.stat(paths[0])).isDirectory
            ) {
              pathPrefix = paths[0];
            }
          ...
          try {
            ({ cid, car } =
              files.length === 1 && paths[0].endsWith(files[0].name)
                ? await NFTStorage.encodeBlob(files[0])
                : await NFTStorage.encodeDirectory(files));
          } catch (err) {
            console.error(err);
            return sendUploadProgress({
              error: `packing files: ${err.message}`,
            });
          }
