...
export function App() {
  ...
  useEffect(() => {
    const handleUploadProgress = async (_, progress) => {
      ...
      if (progress.cid != null) {
        setCid(progress.cid);
        setCidArray((cidArray) => [...cidArray, `ipfs://${progress.cid}\n`]);
  ...
  if (stage === STAGE_REPORTING) {
    // fs.writeFile("./List_of_CIDs.txt", `ipfs://${cid}`).then(function () {
    ...
    return (
      <Layout>
        <Reporter
          cid={cid}
          cidArray={cidArray}
          onClose={() => setStage(STAGE_PICKING)}
        />
      </Layout>
    );
  }
...
function Layout({ children }) {
  return (
    <div className="flex items-center vh-100">
      ...
      <div className="flex-auto h-100 flex">{children}</div>
    </div>
  );
}
